...
db_password <- 'testsql!'
db_name <- 'oudb'
db_host <- 'database-2.cfm7cuoizny6.us-east-2.rds.amazonaws.com'
db_port <- 3306
...
dbWriteTable(oudb, "Cars", Cars[1:30, ])
dbWriteTable(oudb, "Computers", Computers[1:30, ])
dbWriteTable(oudb, "Candy", Candy[1:30, ])
dbReadTable(oudb, "Cars", "Computers", "Candy")

dbWriteTable(oudb, "Toy1", Toy1[1:30, ])
dbWriteTable(oudb, "Toy2", Toy2[1:30, ])
dbWriteTable(oudb, "Toy3", Toy3[1:30, ])
dbReadTable(oudb, "Toy1", "Toy2", "Toy3")
```
...
dbGetQuery(oudb, "SELECT Cars ORDER BY DESC LIMIT 20;")

#Read the other table according to a condition of one of the variables.

dbGetQuery(oudb, "SELECT * FROM Computers")
...
dbGetQuery(oudb, "INSERT INTO Cars (1, 2, 3, 4, 5) VALUES ('Nissan', 'BMW', 'Audi', 'Mercedes', 'NULL');")

dbGetQuery(oudb, "UPDATE Toy1 SET Toy_id_1 = '1'")

dbGetQuery(oudb, "SELECT * FROM Cars")
dbGetQuery(oudb, "SELECT * FROM Toy1")
...
dbGetQuery(oudb,"CREATE TABLE Cars (
  car_id_1 INTEGER, 
  car_id_2 TEXT
  car_id_3 TEXT
  car_id_4 TEXT
...
dbGetQuery(mydb, "SELECT * FROM Cars;")
dbGetQuery(mydb, "DROP TABLE Cars;")
...
dbGetQuery(oudb,"CREATE TABLE Cars (
   car_id_1 INTEGER DEFAULT 0
   );")
dbGetQuery(oudb,"INSERT INTO Cars (car_id_1) VALUES ('1');")
dbGetQuery(oudb, "SELECT * FROM Cars;")

dbGetQuery(oudb, "DROP TABLE Cars;")
...
dbGetQuery(mydb, "ALTER TABLE Cars ADD car_id_31 INTEGER DEFAULT 3")
dbGetQuery(mydb, "ALTER TABLE Cars DROP COLUMN car_id_31;")
...
dbGetQuery(oudb,"CREATE TABLE NewTable (
...
dbGetQuery(oudb, "SELECT car_id_1 FROM Cars WHERE car_id_1 > 0 OR car_id_2 = BMW;")
dbGetQuery(oudb, "SELECT car_id_3 FROM Cars WHERE car_id_3 = AUDI AND car_id_2 = BMW;")
...
dbGetQuery(oudb, "INSERT car_id_4 FROM Cars;")
dbGetQuery(oudb, "SELECT DISTINCT car_id_4 FROM Cars;")
```
...
dbGetQuery(oudb, "INSERT INTO Cars (car_id_32) VALUES ('10');")
dbGetQuery(oudb, "INSERT INTO Toy2 (car_id_32) VALUES ('10');")
dbGetQuery(mydb,"SELECT car_id_32 AS Cars, car_id_32 AS Toy2 
           FROM Cars AS  
           JOIN Toy2)
